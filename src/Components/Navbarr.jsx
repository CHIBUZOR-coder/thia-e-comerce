...
const Navbar = ({ handleSearch, handleCart, triggerRender, isSticky }) => {
  ...
  // useEffect(() => {
  //   console.log('navCart:', cartCount)
  // }, [cartCount])

  useEffect(() => {
    // console.log('NavCart:', CartItems)
    triggerRender(CartItems)
  }, [CartItems])
  ...
  useEffect(() => {
    const handleStorageChange = () => {
      setForceUpdate(prev => !prev) // Force component to re-render
    }
              ...
              {tittle === 'Shop All' ? (
                <span className='relative flex items-center justify-center gap-1'>
                  ...
                  >
                    ...
                    <Link
                      className={`decorate2 ${
                        location.pathname === '/Kaftn' ? 'text-red-500' : ''
                      }`}
                      to='/Kaftn'
                    >
                      Kaftan
                    ...
                    <Link
                      className={`decorate2 ${
                        location.pathname === '/Bridls' ? 'text-red-500' : ''
                      }`}
                      to='/Bridls'
                    >
                      Bridals
                    ...
                    <Link
                      className={`decorate2 ${
                        location.pathname === '/Matchng' ? 'text-red-500' : ''
                      }`}
                      to='/Matchng'
                    >
                      Matching Set
                    ...
                    <Link
                      className={`decorate2 ${
                        location.pathname === '/Kidis' ? 'text-red-500' : ''
                      }`}
                      to='/Kidis'
                    >
                      Kiddies
        ...
        >
                  ...
                  <span className='relative  flex w-24 gap-1'>
                    ...
                    >
                      <Link
                        onClick={handleClose}
                        className={`decorate2 ${
                          location.pathname === '/Akara' ? 'text-red-500' : ''
                        }`}
                        to='/Akara'
                      >
                        Ankara
                      ...
                      <Link
                        onClick={handleClose}
                        className={`decorate2 ${
                          location.pathname === '/Ashebi' ? 'text-red-500' : ''
                        }`}
                        to='/Ashebi'
                      >
                        Ashoebi
          ...
          <div className='flex  w-3/4 justify-end items-start px-4 '>
            <div className='text-lg flex h-1/2.5 text-white sm:flex gap-8 md:hidden'>
              <Link
                onClick={togglee}
                to={`${user?.role ? '/Account' : '/Login'}`}
                href='/Login'
                className='flex items-baseline'
              >
                <FaUser className='text-[25px]'></FaUser>
              </Link>

              <span className='flex  justify-center relative   text-[28px]relative   '>
                <div
                  onClick={DoubleToggle}
                  className=' cursor-pointer z-20 absolute w-full h-full'
                ></div>
                <div className='relative flex flex-col justify-center items-center'>
                  <FaShoppingBag className='text-[25px] '></FaShoppingBag>
                  <p className='absolute  text-[18px] top-[2px] font-semibold  text-black'>
                    {(cartCount && cartCount) || 0}
                  </p>
                </div>
              </span>
              ...
              <div className='flex '>
                <FaSearch
                  onClick={handleSearch}
                  className=' text-[25px] text-white cursor-pointer md:hidden'
                />
              </div>
            </div>
          </div>
